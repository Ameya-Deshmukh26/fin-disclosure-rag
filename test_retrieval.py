...
def top_k_similar(query_vector: np.ndarray, doc_vectors: np.ndarray, k: int) -> list:
    scores = np.dot(doc_vectors, query_vector) / (np.linalg.norm(doc_vectors, axis=1) * np.linalg.norm(query_vector))
...
print("Result:", result)
print("Is it sorted descending by score?", [round(s,3) for _, s in result])
